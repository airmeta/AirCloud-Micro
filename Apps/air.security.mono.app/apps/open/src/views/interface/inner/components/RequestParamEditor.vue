<template>
  <!-- 如果父级是数组，渲染为块状列表，默认仅保留一个数组元素，不可增删 -->
  ...
  <div v-else class="param-editor">
    ...
    >
      ...
      <el-table-column prop="isRequired" label="必填" width="60">
        <template #default="{ row }">
          <el-checkbox v-if="row.type !== 'array'" v-model="row.isRequired" />
        </template>
      </el-table-column>
      <el-table-column prop="description" label="描述" min-width="150">
        <template #default="{ row }">
          <el-input v-if="row.type !== 'array'" v-model="row.description" size="small" placeholder="描述" />
        </template>
      </el-table-column>
...
import type { InterfaceRequestParameter } from '../../common/interface_parameter.dto'

const props = defineProps<{
  modelValue: InterfaceRequestParameter[]
  label?: string
  depth?: number
  parentType?: string
...
const emit = defineEmits<{
  (e: 'update:modelValue', value: InterfaceRequestParameter[]): void
}>()

function ensureArrayElementsObject(list: InterfaceRequestParameter[]) {
  if (!list) return
...
function getTypeOptions() {
  // 如果父级是 array，则子级只能是 object
  if (props.parentType === 'array') {
    return [
      { label: 'object', value: 'object' },
    ]
  }
  // 如果父级是 object，或父级是 object 且其父级是 array，则子级只能是基础类型
...
let keyCounter = 0
function genKey() {
  return `__req_${Date.now()}_${keyCounter++}`
}
