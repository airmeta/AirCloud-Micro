...
import MenuItemContent from "@root/base/layouts/components/Menu/components/MenuItemContent.vue";
...
type VisibleRouteRecord = RouteRecordRaw & {
...
// 可见的唯一子节点
const onlyOneChild = ref<VisibleRouteRecord>({ path: "" } as VisibleRouteRecord);
...
function hasOneShowingChild(children: RouteRecordRaw[] = [], parent: RouteRecordRaw) {
  ...
  const showingChildren = children.filter((route: RouteRecordRaw) => {
    if (!route.meta?.hidden) {
      onlyOneChild.value = route as VisibleRouteRecord;
      return true;
    }
    return false;
  });
