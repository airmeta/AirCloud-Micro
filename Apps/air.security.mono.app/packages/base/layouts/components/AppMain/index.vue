...
import { useSettingsStore, useTagsViewStore } from "@root/base/store";
import variables from "../../../styles/variables.module.scss";
import Error404 from "@root/base/views/error/404.vue";
...
const currentComponent = (component: Component, route: RouteLocationNormalized) => {
  ...
  let wrapper = wrapperMap.get(componentName);
    ...
    wrapper = {
      ...
      render: () => {
        try {
          // Ensure a single element root for Transition by wrapping the page component
          return h('div', { class: 'route-wrapper', style: { height: '100%' } }, [h(component)]);
        } catch (error) {
          console.error(`Error rendering component for route: ${componentName}`, error);
          return h('div', { class: 'route-wrapper', style: { height: '100%' } }, [h(Error404)]);
        }
      },
    };
