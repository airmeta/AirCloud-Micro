...
import qs from "qs";
import { ApiCodeEnum } from "@root/base/enums/api/code-enum";
import { redirectToLogin, getTicket } from "@root/base/utils/auth";
import { useSettingsStore } from "@root/base/store";
import { v4 as uuidv4 } from 'uuid';
import { SignatureUtil } from '@root/base/utils/signature';
...
httpRequest.interceptors.response.use(
  (response: AxiosResponse<ApiResponse>) => {
    ...
    return Promise.reject(new Error(errors || "Business Error"));
  },
  async (error) => {
