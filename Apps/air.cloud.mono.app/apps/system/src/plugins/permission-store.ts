...
import {usePermissionStore as baseUsePermissionStore} from "@root/base/store/modules/permission-store";
...
const transformRoutes = (routes: authorityTree[], isTopLevel: boolean = true): RouteRecordRaw[] => {
  return routes.map((route) => {
    ...
    if (!processedComponent) {
      ...
      normalizedRoute.component =processedComponent === "Layout"? Layout
...
export const usePermissionStore = defineStore("system-permission", () => {
  return {
    routes:()=>{
      return baseUsePermissionStore().routes;
    },
    mixLayoutSideMenus:()=>{
      return baseUsePermissionStore().mixLayoutSideMenus;
    },
    isRouteGenerated:()=>{
      return baseUsePermissionStore().isRouteGenerated;
    },
    generateRoutes:async ()=>{
      return await baseUsePermissionStore().generateRoutes(transformRoutes);
    },
    setMixLayoutSideMenus:(parentPath: string) => {
      return baseUsePermissionStore().setMixLayoutSideMenus(parentPath);
    },
    resetRouter:() => {
      return baseUsePermissionStore().resetRouter();
  }
  };
});
