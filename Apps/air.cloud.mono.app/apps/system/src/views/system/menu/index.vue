<template>
  <div class="app-container">
    ...
    <air-table ref="dataTableRefLocal" :queryFormRef="queryFormRef" v-loading="loading" row-key="id" :page-size="0"
      :request="MenuAPI.query" :after-request="afterRequest" :tree-props="{
        children: 'children',
        hasChildren: 'hasChildren',
      }" :showExpandButtons="true" @row-click="handleRowClick" :columns="tableColumns">
      ...
      <template #operation="{ row }">
        <el-button type="primary" link size="small" icon="edit" @click.stop="handleOpenDialog(undefined, row.id)">
          编辑
        </el-button>
    ...
    <el-drawer v-model="dialog.visible" :title="dialog.title" :size="drawerSize" @close="handleCloseDialog">
      <MenuForm ref="menuFormRefLocal" :formData="formData" :MenuTypeEnum="MenuTypeEnum"
        @menu-type-change="handleMenuTypeChange" @success="handleFormSuccess" @cancel="handleCloseDialog" />
    </el-drawer>
  </div>
</template>
