<template>
    <div class="dict-container" style="display:flex;height:100%;">
        <div class="dict-left"
            style="width:260px;border-right:1px solid var(--el-border-color);overflow:auto;padding:12px;box-sizing:border-box">
            ...
            <el-menu :default-active="selectedId" class="el-menu-vertical" @select="onLeftSelect">
                <el-menu-item style="height: 40px;" v-for="item in filterData" :key="item.id" :index="item.id">
                    <div style="display: flex;justify-content: space-between;align-items: center;width:100%;">
                        ...
                        <div style="font-size:10px;color:#323232;" :title="item.code">{{ item.code.length>10?item.code.substring(0,10) + '...' : item.code }}</div>
                    </div>
                </el-menu-item>
            </el-menu>
        </div>
        <div class="dict-right" :style="{ width: 'calc(100% - 260px)', padding: '12px' }">
            <div v-if="selectedId">
                <div class="dict_data_parent">
                    <div>字典名称: <span>{{ selectDict.label }}</span></div>
                    <div>字典编码: <span>{{ selectDict.code }}</span></div>
                    <div>字典值: <span>{{ selectDict.value }}</span></div>
                </div>
                    ...
                    <template #table-query-form>
                        <el-form ref="queryFormRef" :model="queryParams" :inline="true">
                            <el-form-item label="名称" prop="label">
                                <el-input v-model="queryParams.label" placeholder="请输入名称检索" clearable />
                            </el-form-item>
...
const selectDict = computed(() => {
    return baseList.value.find((it: any) => it.id === selectedId.value);
});
